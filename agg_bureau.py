...
if __name__ == "__main__":
    ...
    nan_cols = ['DAYS_CREDIT_ENDDATE','DAYS_ENDDATE_FACT','AMT_CREDIT_MAX_OVERDUE',
            'AMT_CREDIT_SUM','AMT_CREDIT_SUM_DEBT','AMT_CREDIT_SUM_LIMIT',
            'AMT_ANNUITY','DAYS_CREDIT_ENDDATE_MINUS_ENDDATE_FACT']
    cols = raw.columns
    isTime = True
    ...
    for col in tqdm(cols):
        if col in nan_cols:
            pad = missToComp(raw, col, 'SK_ID_CURR', 'TARGET', dispersed=False,isTime=isTime)
    ...
    temp_merge_repay_state_df = getComp(temp_merge_repay_state_df,id_name,target_name,str_columns)
    ...
    temp_merge_currency_df = getComp(temp_merge_currency_df,id_name,target_name,str_columns)
    # temp_merge_repay_state_df = getPCAResult(temp_merge_repay_state_df,target_name,id_name)
    ...
    merge_df = merge_df[high_corr_cols]